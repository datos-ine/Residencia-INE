...
Desde la versión 1.2.0 se agregó la función `filter_out()` para especificar condiciones de filas que se van a eliminar.
...
datos |> 
    filter_out(casos < 200)
```

Las dos estructuras de código devuelven la misma tabla, la primera mediante `filter()` con casos > 200 y la segunda con `filter_out()` negando las filas con casos < 200.   
...
También desde la versión 2.1.0 de dplyr, aparecieron tres nuevas funciones relacionadas con `case_when()`:

- `replace_when()` para actualizar un vector existente según condiciones lógicas. Puede considerarse una versión mejorada de `replace()` de R base.

- `recode_values()` para crear un nuevo vector asignando valores nuevos a todos los valores antiguos. 

- `replace_values()` para actualizar un vector existente asignando valores nuevos a algunos valores antiguos.
