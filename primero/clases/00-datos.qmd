...
## Los datos en epidemiología

En epidemiología, los datos constituyen una representación parcial y situada de fenómenos relacionados con la salud y la enfermedad. Un dato no equivale al fenómeno que pretende representar, siempre es un "recorte", resultado de un proceso mediante el cual una característica, fenómeno o constructo teórico es definido, operacionalizado, medido y registrado para una determinada unidad de análisis. Por esta razón, una base de datos debe entenderse como el producto de decisiones conceptuales, metodológicas, técnicas e institucionales. [Blog Mailen García](https://politicadeldato.substack.com/p/esa-cosa-llamada-datos)

El proceso puede representarse esquemáticamente como:

![](img/00/proceso_dato.png){fig-align="center" .lightbox}

Cada transición implica decisiones. Por ejemplo, estudiar la diabetes requiere definir qué se entenderá por diabetes, qué criterios permitirán identificar un caso, qué población será considerada, qué instrumento se utilizará y cómo se codificará el resultado. La variable diabetes = 1/0 o TRUE/FALSE o Si/No, que finalmente observamos en una tabla constituye, por lo tanto, el resultado de una cadena de decisiones que antecede al análisis estadístico.

Esta perspectiva permite distinguir entre dato, información y conocimiento. El dato es un registro de una observación o medición, la información surge cuando esos datos son organizados y relacionados con una pregunta o problema y el conocimiento implica interpretar esa información dentro de un marco conceptual y metodológico. En consecuencia, disponer de grandes cantidades de datos no garantiza, por sí mismo, producir conocimiento epidemiológico. La calidad del análisis depende también de la pertinencia de las preguntas, de las definiciones utilizadas, de la calidad de las mediciones y de los supuestos que orientan su interpretación.

La propia estructura de una base de datos expresa parte de esas decisiones. Cuando una tabla organiza filas como unidades de análisis y columnas como variables, estamos adoptando una determinada representación de la realidad. Las unidades de análisis podrían ser personas, consultas, nacimientos, defunciones, muestras ambientales o establecimientos de salud, y una misma realidad puede requerir más de una tabla para ser adecuadamente representada. La estructura de esos datos puede estar anidada, repetida, pertenecer a un seguimiento o tener cierta jerarquía, por lo que antes de analizarlos será necesario preguntarse qué representa cada fila, qué representa cada variable y cuál es la relación entre las unidades registradas.

Esta consideración es particularmente importante al trabajar con fuentes secundarias y sistemas de información sanitaria. Un registro de vigilancia epidemiológica, una historia clínica, una encuesta poblacional o una base administrativa no son equivalentes: fueron producidos con objetivos, poblaciones, procedimientos de medición y criterios de registro diferentes. La ausencia de un evento en una base tampoco necesariamente significa ausencia del fenómeno en la población. Puede reflejar, entre otras posibilidades, diferencias en el acceso a los servicios, criterios de notificación, cobertura del sistema o prácticas institucionales de registro.

Desde esta perspectiva, el análisis epidemiológico es un diálogo entre ideas y datos. Las preguntas y los conceptos orientan qué datos se producen, seleccionan y analizan; a su vez, los datos pueden cuestionar, modificar o refinar las ideas iniciales. Esta relación evita concebir el análisis estadístico como una operación puramente técnica en la que un método aplicado a una base produce automáticamente una conclusión. La elección de variables, la definición de poblaciones, la categorización de exposiciones y desenlaces, la selección de modelos y la interpretación de sus resultados forman parte del razonamiento epidemiológico.

En este sentido, veremos que utilizar softwares como el lenguaje R no significa simplemente aprender a manipular tablas o ejecutar procedimientos estadísticos. Significa adquirir herramientas para explorar, transformar, describir y modelizar representaciones de fenómenos epidemiológicos, manteniendo siempre presente cómo fueron construidos esos datos, qué información contienen, qué información excluyen y bajo qué supuestos pueden utilizarse para responder una pregunta de investigación o vigilancia.

## Conceptos técnicos de un conjunto de datos
...
La [Oficina de Ética de Investigación Humana de la Universidad de Carolina del Norte](https://research.unc.edu/2020/05/01/de-identified-coded-or-anonymous-how-do-i-know/) define 4 tipos de archivos de datos en función a la identificación personal:
...
![](img/00/gestion.PNG){fig-align="center"}
