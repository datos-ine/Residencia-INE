...
*"Actúa como un analista de datos senior en salud experto en R y tidyverse. Solo responde con el código R requerido para crear una función reutilizable y una breve explicación de por qué funciona. 
La función debe llamarse convertir_fechas_dmy, aceptar como único argumento un data frame, y devolver el data frame con todas las variables de tipo character con formato dd/mm/aaaa convertidas a tipo Date utilizando lubridate::dmy(). La función debe utilizar la estructura dplyr::mutate(across(..)) y debe ser robusta que cumplen dos condiciones simultáneas: 1) su tipo de dato es character Y 2) su nombre contiene la subcadena fecha en minúsculas."*
