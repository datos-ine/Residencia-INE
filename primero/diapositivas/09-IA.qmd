...
*"Actúa como un analista de datos senior en epidemiología experto en R y tidyverse. Solo responde con el código R requerido y una breve explicación de cómo funciona. Necesito código para el data frame rita_prueba que me permita convertir todas las variables de fecha que tengan el formato dd/mm/aaaa en Date. Tené en cuenta que algunos valores de observaciones pueden ser NA u otro valor de texto"* 
...
La función debe llamarse convertir_fechas_dmy, aceptar como único argumento un data frame, y devolver el data frame con todas las variables de tipo character con formato dd/mm/aaaa convertidas a tipo Date utilizando dmy() de lubridate. Utiliza require() para activar el o los paquetes que necesites asegurándote de tenerlos instalados, de lo contrario los instalas. La función debe utilizar la estructura mutate(across()) de dplyr y debe ser robusta para que cumpla dos condiciones simultáneas: 1) su tipo de dato es character o factor y 2) su estructura es dd/mm/aaaa (usa str_detect() de string para el patrón). Tené en cuenta que algunos valores de observaciones pueden ser NA u otro valor como S/D"*
...
```{r, echo=T, eval=F}
convertir_fechas_dmy <- function(df) {
  
  # --- Bloque de Gestión de Paquetes (Método if/library) ---
  
  # 1. Verificar e instalar dplyr
  ...
  # 3. Verificar e instalar lubridate
  ...
  # 5. Patrón regex para identificar formato dd/mm/aaaa
  patron_fecha <- "^\\d{1,2}/\\d{1,2}/\\d{4}$"
  
  resultado <- df %>%
    mutate(across(
      where(~ (is.character(.) || is.factor(.)) && 
              any(str_detect(., patron_fecha), na.rm = TRUE)),
      ~ {
...
Definición de la función: `convertir_fechas_dmy <- function(df) {` - Crea una función llamada **convertir_fechas_dmy** que acepta un data frame como único argumento.

Verificación de paquete dplyr: `if (!requireNamespace("dplyr", quietly = TRUE)) {` - Comprueba si el paquete `dplyr` está instalado en el sistema sin mostrar mensajes.

Instalación de dplyr: `install.packages("dplyr", dependencies = TRUE)` - Si `dplyr` no está instalado, lo instala automáticamente con todas sus dependencias.

Carga de dplyr: `require(dplyr)` - Carga el paquete `dplyr` en el entorno de trabajo actual.

Repetición del proceso: El mismo patrón de verificación-instalación-carga se aplica para los paquetes `lubridate` y `stringr`.
...
Definición del patrón regex: `patron_fecha <- "^\\d{1,2}/\\d{1,2}/\\d{4}$"` - Crea una expresión regular que busca exactamente el formato *dd/mm/aaaa* donde "dd" y "mm" pueden tener 1 o 2 dígitos, y "aaaa" siempre tiene 4 dígitos.

Inicio de la transformación: `resultado <- df |> ` - Toma el data frame de entrada y lo prepara para encadenar operaciones con el operador pipe ( |> ).

Selección de columnas: `mutate(across(where(~ (is.character(.) || is.factor(.)) &&` - Inicia la modificación de columnas, seleccionando solo aquellas que cumplan dos condiciones simultáneas. Aquí usamos `||` y `&&` para decisiones con valores únicos (condiciones de control).
                                          
Primera condición: `is.character(.) || is.factor(.)` - La columna debe ser de tipo  character O de tipo factor.
...
Segunda condición: `any(str_detect(., patron_fecha), na.rm = TRUE)))` - Al menos un valor en la columna debe coincidir con el patrón de fecha definido, ignorando valores NA.
                             
Función de transformación: `~ {` - Inicia una función anónima que se aplicará a 
cada columna seleccionada.
                                
Conversión de factores: `if (is.factor(.x)) { .x <- as.character(.x) }` - Si la columna es un factor, la convierte a character para poder procesarla con `str_detect` y `dmy`.
                               
Conversión a fecha: `dmy(.x, quiet = TRUE)` - Utiliza la función `dmy()` de lubridate para convertir strings en formato día/mes/año a objetos *Date* de R, silenciando las advertencias.
                                 
Retorno del resultado: `return(resultado)` - Devuelve el data frame modificado 
con las columnas de fecha convertidas a tipo Date.
